...
# Versão 9 - Adicionada autenticação via cookies do navegador para evitar bloqueios
# Autor: Manus AI
# ===================================================================================
...
install_dependencies() {
    ...
    if [ ${#missing_deps[@]} -gt 0 ]; then
        ...
        if [[ "$choice" == "s" || "$choice" == "S" ]]; then
            ...
            if [[ " ${missing_deps[*]} " =~ " ffmpeg " ]] || [[ " ${missing_deps[*]} " =~ " curl " ]]; then
                echo -e "\n${C_BLUE}Instalando 'ffmpeg' e 'curl'...${C_NC}"; sudo apt-get install -y ffmpeg curl
            fi
...
start_download( ) {
    ...
    read -p "Escolha uma opção (1 ou 2): " FORMAT_CHOICE

    local FORMAT_FOLDER=""
    local cmd=("yt-dlp")

    case $FORMAT_CHOICE in
        1) FORMAT_FOLDER="MP4"; cmd+=(-f 'bestvideo+bestaudio/best' --merge-output-format mp4);;
        2) FORMAT_FOLDER="MP3"; cmd+=(-x --audio-format mp3 --audio-quality 0);;
    ...
    local OUTPUT_TEMPLATE="${HOME}/Músicas/${FORMAT_FOLDER}/%(playlist_title,title)s/%(playlist_index,NA)s - %(title)s.%(ext)s"
    cmd+=(-o "$OUTPUT_TEMPLATE" "$MEDIA_URL")

    echo -e "\n${C_BLUE}Iniciando download...${C_NC}"
    echo -e "Os arquivos serão salvos em: ${C_YELLOW}${HOME}/Músicas/${FORMAT_FOLDER}/${C_NC}"

    # Adiciona um pequeno atraso entre os downloads para parecer mais "humano"
...
install_dependencies
while true; do
    echo -e "\n--- ${C_YELLOW}YouTube Downloader v9 (Cookie Auth)${C_NC} ---"
